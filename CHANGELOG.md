...
- 点赞可能被重复计数：修复同一访客重复点赞，或点赞按钮与「评论同时点赞」勾选叠加时计数多加的问题；点赞 Cookie 对齐 WordPress 标准

**优化**

- 苹果 Coverflow 首张封面的背景取色更快，不再等待全部图片加载；同域封面复用浏览器缓存，跨切换不再出现明显延迟

**变更**

- 移除按浏览器 User-Agent 的设备降级：手机端苹果 Coverflow 不再被强制压成低一档样式，各平台观感统一；导航栏溢出时的「左右箭头 / 手指滑动」两套行为改由指针类型（触摸或鼠标）决定；Bootstrap 工具提示（tooltip）在所有设备统一启用
