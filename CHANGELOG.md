...
**安全修复**
- 删除两个未认证后门（`pf_rest_api`、`pf_assistance`）
...
- 文章 Meta 保存竞态：在数据加载完成前发布可能用默认值覆盖已保存数据
- 扁平卡片流轮播切换按钮 hover 抖动
- 瀑布流列表角标退出位置跳变
- 搜索按钮第二次点击失效、无缩略图搜索结果布局错乱
- 主题设置页配置读取改为绝对路径，修复部分环境下后台设置不显示的问题

**变更**
- 文章图片字段合并：删除「滚动图片」，原「日志头图」改名「文章横幅图」——文章页顶部大图、滚动图片轮播、头图背景轮播统一使用该字段
- **升级后建议运行迁移工具**（`migrate-comment-faces.php` 任务二）将旧图片字段数据复制到新字段
