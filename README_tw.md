...
- **深色模式** - 淺色 / 深色 / 跟隨系統三段切換，預設跟隨系統
- **滾動圖** - 內建 8 種滾動圖風格（3D 封面流、扁平、扁平卡片流、滾動圖片、頭圖背景等），首頁與分類頁可獨立設定
- **無刷新載入** - 非同步載入文章，支援全域開關
- **文章目錄** - 自動識別文章中的二級／三級標題，產生目錄浮動顯示於側邊欄
- **固定側邊欄** - 可選文章／頁面側邊欄在捲動時固定顯示
- **側邊欄小工具** - 側邊欄數量可自訂，內建站長資訊、標籤籤雲、輕部落格、熱門文章小工具

**內容創作**

- **UI 樣式區塊** - 內建 10 種內容區塊與 4 種小工具（提示框、摺疊、下拉選單、互動視窗、下載按鈕、輪播、文章展示、回覆可見、B站影片等），編輯器內直接插入
- **說說功能** - 簡短的說說功能，隨時隨地記錄您的想法
- **程式碼高亮** - 文章內程式碼自動高亮，支援一鍵複製
- **卡片-角標** - 文章縮圖可設定角標文字與顏色

**讀者互動**

- **評論表情** - 本主題內建評論表情功能，無需外掛即可在後台設定
- **按讚** - 每篇文章均提供按讚功能，可展示訪客最喜歡的文章列表（後台可修改按讚數據）
- **內容回覆可見** - 指定某些內容需要讀者評論後才可查看，讓讀者與你互動（可設定為要求使用者註冊登入並評論後才顯示某些內容）
- **回覆下載** - 可設定要求使用者評論文章後才提供下載位址
- **語音朗讀** - 整合語音朗讀功能，為您朗讀文章

**分享與支持**

- **產生封面二維碼** - 每篇文章均可產生二維碼，用於分享至微信等社群平台
- **打賞** - 允許透過多種途徑打賞，例如：支付寶二維碼、微信二維碼、PayPal 連結

**網站管理**

- **主題自動更新** - 內建自動更新檢查，新版本發布後可在後台直接更新
- **多語言** - 安裝 [niRvana-translate 多語言翻譯外掛](https://github.com/michaelliunsky/niRvana-translate)，享受極速多語言切換
