...
$content = array("slider_img" => get_post_meta($pid, "分类slider图片地址", true),"head_img" => get_post_meta($pid, "日志头图", true),"title" => single_tag_title('', false),"category" => 'Tag · 标签'
);?>
...
if (_opt('category_hide_sidebar')=='checked') {
    ...
    $leftClass = 'col-sm-8 col-md-9 col-lg-9_5';
    $rightClass = 'col-sm-4 col-md-3 col-lg-2_5 hidden-xs';
}?>
...
include('nav-category.php');?>
            <div class="col-xs-12">
                <div class="row">
                    <h2 class="widgettitle tag-title"><i class='fa fa-tags' aria-hidden='true'></i> <?php echo single_tag_title('', false); ?></h2>
                </div>
                <div class="row">
                    <div class="row postLists <?php echo $show_type; ?> waterfall" height-to="sidebar"><?php
global $wp_query;
while (have_posts()) {
    the_post();?>
                        <div class="col-xxs-6 col-xs-4 col-sm-6 col-md-4 col-lg-3 post-card-wrapper"><?php include('postlist-post.php'); ?></div><?php
}?>
                    </div>
                </div>
            </div>
