...
function update_option_by_RestAPI($data)
{
    if (current_user_can('manage_options')) {
        ...
        return array('state' => false, 'error' => __('PANDA Studio framework 无法执行此操作，原因：您没有进行此操作的权限', 'niRvana'));
    }
}
...
function wp_query_by_RestAPI($data)
{
    ...
    if (current_user_can('manage_options')) {
        ...
        return array('label' => __('无权限', 'niRvana'), 'value' => '0');
    }
}
    ...
    echo '<script type="text/javascript" src="'.get_stylesheet_directory_uri().'/pandastudio_framework/assets/js/vue.js"></script><script type="text/javascript" src="'.get_stylesheet_directory_uri().'/pandastudio_framework/assets/js/element-ui.js"></script><script type="text/javascript" src="'.get_stylesheet_directory_uri().'/pandastudio_framework/assets/js/element-locale/'.$locale.'.js"></script><script type="text/javascript">if (window.ELEMENT && window.ELEMENT.lang) { window.ELEMENT.i18n(window.ELEMENT.lang[\''.$locale.'\']); }</script><link rel="stylesheet" type="text/css" href="'.get_stylesheet_directory_uri().'/pandastudio_framework/assets/css/'.$color.'.css"><link rel="stylesheet" type="text/css" href="'.get_stylesheet_directory_uri().'/pandastudio_framework/assets/css/rewrite.css"><link rel="stylesheet" type="text/css" href="'.get_stylesheet_directory_uri().'/pandastudio_framework/assets/css/font-awesome.css"><div id="vue_rest" class="wrap">';?><template><span v-if="false" style="color: red;"><?php echo esc_html__('您的浏览器不支持 ECMAScript 5，请更换至 IE9 及以上版本', 'niRvana'); ?></span></template><template><el-tabs v-model="tabIndex" v-loading="loading" v-show="show"><el-tab-pane v-for="tab in tabs" v-if="gear_show(tab.gear_name,tab.gear_value)"><span slot="label"><i :class="tab.icon" class="fa"></i> {{tab.title}}</span><el-form ref="form" :label-width="tab.labelWidth" style="padding-right: 15px;" onsubmit="return false;"><el-form-item v-for="component in tab.content" v-if="gear_show(component.gear_name,component.gear_value)"><span slot="label" v-html="component.label"></span>
                    ...
                    <div v-if=" component.type == 'multi_uploader' ">
                        ...
                        <div class="el-upload el-upload--picture-card">
                            <div class="hover"><el-tooltip class="item" effect="dark" content="<?php echo esc_attr__('输入外链', 'niRvana'); ?>" placement="top"><i class="el-icon-edit" @click="multiMediaUpload_input(component.name)"></i></el-tooltip><span style="width:15px;display: inline-block;"></span><el-tooltip class="item" effect="dark" content="<?php echo esc_attr__('批量上传', 'niRvana'); ?>" placement="top"><i class="el-icon-upload2" @click="multiMediaUpload(component.name)"></i></el-tooltip></div>
                            <div class="normal"><i class="el-icon-plus"></i></div>
                        </div>
                        <div v-if="component.value.length > 1"><el-button @click="removeAllMultiUpload(component.name)" type="danger" :plain="true" size="mini" icon="el-icon-delete" class="subcontentRemove"><?php echo esc_html__('全部移除', 'niRvana'); ?></el-button></div>
                    </div>
                    ...
                    <div v-if=" component.type == 'multitypes' ">
                        <div v-if="component.displayAsTable == false"><el-form-item v-for="(subValue , index) in component.value" :label="component.subLabel + ' ' + (index+1)" :label-width="component.subLabelWidth">
                                <div v-for="subcomponent in component.types" style="margin-bottom: 5px;">
                                    ...
                                    <div v-if=" subcomponent.type == 'select' " class="subcomponent"><el-select size="small" v-model="subValue[subcomponent.name]" :placeholder="subcomponent.placeholder ? subcomponent.placeholder : '<?php echo esc_attr__('请选择', 'niRvana'); ?>'" clearable :multiple="subcomponent.multiple"><el-option v-for="select in subcomponent.selects" :key="select.value" :label="select.label" :value="select.value"><span style="float: left">{{ select.label }}</span><span style="float: right; color: #C0CCDA; font-size: 13px;margin-left:15px;">{{ select.description }}</span></el-option></el-select></div>
                                    <div v-if=" subcomponent.type == 'categorySelect' " class="subcomponent"><el-select size="small" v-model="subValue[subcomponent.name]" placeholder="<?php echo esc_attr__('请选择文章分类', 'niRvana'); ?>" clearable><el-option v-for="select in categorySelect()" :key="select.value" :label="select.label" :value="select.value"><span style="float: left">{{ select.label }}</span><span style="float: right; color: #C0CCDA; font-size: 13px;margin-left:15px;">ID: {{ select.value }}</span></el-option></el-select></div>
                                    <div v-if=" subcomponent.type == 'categoriesSelect' " class="subcomponent"><el-select size="small" v-model="subValue[subcomponent.name]" placeholder="<?php echo esc_attr__('请选择至少一个文章分类', 'niRvana'); ?>" multiple><el-option v-for="select in categorySelect()" :key="select.value" :label="select.label" :value="select.value"><span style="float: left">{{ select.label }}</span><span style="float: right; color: #C0CCDA; font-size: 13px;padding-right: 20px;margin-left:15px;">ID: {{ select.value }}</span></el-option></el-select></div>
                                    <div v-if=" subcomponent.type == 'singleSelect' "><el-select size="small" v-model="subValue[subcomponent.name]" :placeholder="component.placeholder ? component.placeholder : '<?php echo esc_attr__('输入关键字来筛选文章', 'niRvana'); ?>'" filterable remote clearable :remote-method="querySinglePosts" :loading="singleSelect.loading" @visible-change="queryExistValue($event,subValue[subcomponent.name])"><el-option v-for="select in singleSelect.query" :key="select.value" :label="select.label" :value="select.value"><span style="float: left">{{ select.label }}</span><span style="float: right; color: #C0CCDA; font-size: 13px;margin-left:15px;">ID: {{ select.value }}</span></el-option></el-select></div>
                                ...
                                </div><el-button :plain="true" size="small" icon="el-icon-arrow-up" @click="multitypeMoveUp(component.value,index)" :disabled="index == 0"><?php echo esc_html__('上移', 'niRvana'); ?></el-button><el-button :plain="true" size="small" icon="el-icon-arrow-down" @click="multitypeMoveDown(component.value,index)" :disabled="index == component.value.length - 1"><?php echo esc_html__('下移', 'niRvana'); ?></el-button><el-button @click="removeMultiTypes(component.name,index)" type="danger" :plain="true" size="small" icon="el-icon-delete" class="subcontentRemove"><?php echo esc_html__('移除', 'niRvana'); ?></el-button>
                        ...
                        <div v-if="component.displayAsTable == true"><el-table size="mini" :data="component.value" style="width: 100%" border><el-table-column type="index" width="55" fixed></el-table-column><el-table-column :prop="subcomponent.name" :label="subcomponent.label ? subcomponent.label : subcomponent.name" v-for="subcomponent in component.types" min-width="265"><template scope="scope">
                                        ...
                                        <div v-if="subcomponent.type == 'uploader'" class="subcomponent"><el-popover placement="bottom" trigger="hover" :disabled="scope.row[subcomponent.name] && !subcomponent.showImage"><el-input v-model="scope.row[subcomponent.name]" :placeholder="subcomponent.placeholder ? subcomponent.placeholder : '<?php echo esc_attr__('点击按钮上传或在此处粘贴外链地址', 'niRvana'); ?>'" size="small" slot="reference"><el-button @click="mediaUpload(component.name,scope.$index,subcomponent.name)" icon="el-icon-upload2" slot="prepend"></el-button><el-button @click="scope.row[subcomponent.name] = ''" icon="el-icon-close" slot="append" v-if="scope.row[subcomponent.name]"></el-button></el-input><img class="uploader_show_image" :src="scope.row[subcomponent.name]" background /></el-popover></div>
                                        ...
                                        <div v-if=" subcomponent.type == 'select' " class="subcomponent"><el-select v-model="scope.row[subcomponent.name]" :placeholder="subcomponent.placeholder ? subcomponent.placeholder : '<?php echo esc_attr__('请选择', 'niRvana'); ?>'" size="small" clearable :multiple="subcomponent.multiple"><el-option v-for="select in subcomponent.selects" :key="select.value" :label="select.label" :value="select.value"><span style="float: left">{{ select.label }}</span><span style="float: right; color: #C0CCDA; font-size: 13px;margin-left:15px;">{{ select.description }}</span></el-option></el-select></div>
                                        <div v-if=" subcomponent.type == 'categorySelect' " class="subcomponent"><el-select v-model="scope.row[subcomponent.name]" placeholder="<?php echo esc_attr__('请选择文章分类', 'niRvana'); ?>" clearable size="small"><el-option v-for="select in categorySelect()" :key="select.value" :label="select.label" :value="select.value"><span style="float: left">{{ select.label }}</span><span style="float: right; color: #C0CCDA; font-size: 13px;margin-left:15px;">ID: {{ select.value }}</span></el-option></el-select></div>
                                        <div v-if=" subcomponent.type == 'categoriesSelect' " class="subcomponent"><el-select v-model="scope.row[subcomponent.name]" placeholder="<?php echo esc_attr__('请选择至少一个文章分类', 'niRvana'); ?>" multiple size="small"><el-option v-for="select in categorySelect()" :key="select.value" :label="select.label" :value="select.value"><span style="float: left">{{ select.label }}</span><span style="float: right; color: #C0CCDA; font-size: 13px;padding-right: 20px;margin-left:15px;">ID: {{ select.value }}</span></el-option></el-select></div>
                                        <div v-if=" subcomponent.type == 'singleSelect' "><el-select v-model="scope.row[subcomponent.name]" :placeholder="component.placeholder ? component.placeholder : '<?php echo esc_attr__('输入关键字来筛选文章', 'niRvana'); ?>'" filterable remote clearable :remote-method="querySinglePosts" :loading="singleSelect.loading" @visible-change="queryExistValue($event,scope.row[subcomponent.name])" size="small"><el-option v-for="select in singleSelect.query" :key="select.value" :label="select.label" :value="select.value"><span style="float: left">{{ select.label }}</span><span style="float: right; color: #C0CCDA; font-size: 13px;margin-left:15px;">ID: {{ select.value }}</span></el-option></el-select></div>
                ...
                </el-form-item><el-form-item :label-width="tab.labelWidth"><el-button size="small" type="primary" icon="el-icon-check" @click="updateOption" v-if="showSaveBtn(tab.content)"><?php echo esc_html__('保存全部', 'niRvana'); ?></el-button></el-form-item></el-form></el-tab-pane></el-tabs></template></div>
