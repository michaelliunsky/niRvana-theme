var postID = jQuery("#post_ID").val(),
  postType = jQuery("#post_type").val();
jQuery(document).on("click", "#publish", function(t) {
  t.preventDefault(), vue_rest.updateMeta(jQuery(this))
}), jQuery(document).on("click", "#save-post", function(t) {
  t.preventDefault(), vue_rest.updateMeta(jQuery(this))
}), jQuery(document).on("click", "#post-preview", function(t) {
  vue_rest.updateMeta()
}), jQuery(document).on("click", ".editor-post-publish-button,.editor-post-preview,.editor-post-save-draft,.editor-post-switch-to-draft ", function(t) {
  vue_rest.updateMeta()
}), jQuery.get(pandastudio_framework.route + "pandastudio/framework/get_posttype_and_meta_json", function(t) {
  ...
  window.vue_rest = new Vue({
    ...
    mounted: function() {
      ...
      jQuery.ajax({
        url: pandastudio_framework.route + "pandastudio/framework/get_post_meta",
        type: "POST",
    ...
    methods: {
      updateMeta: function(t) {
        for (var e = {}, n = 0; n < this.tabs.length; n++)
          for (var a = 0; a < this.tabs[n].content.length; a++) null != this.tabs[n].content[a].name & null != this.tabs[n].content[a].value && (e[this.tabs[n].content[a].name] = this.tabs[n].content[a].value);
        var o = this,
          s = {
            postID: postID,
            postMeta: e
        ...
        jQuery.ajax({
          url: pandastudio_framework.route + "pandastudio/framework/update_post_meta",
          type: "POST",
          beforeSend: function(t) {
            t.setRequestHeader("X-WP-Nonce", pandastudio_framework.nonce)
          },
