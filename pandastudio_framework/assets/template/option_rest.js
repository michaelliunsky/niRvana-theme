jQuery.get(pandastudio_framework.route + "pandastudio/framework/get_option_json", function(t) {
  ...
  window.vue_rest = new Vue({
    ...
    mounted: function() {
      ...
      var a = this;
      jQuery.ajax({
        url: pandastudio_framework.route + "pandastudio/framework/get_option",
        type: "POST",
        beforeSend: function(t) {
          t.setRequestHeader("X-WP-Nonce", pandastudio_framework.nonce)
        },
          ...
          for (var n = 0; n < a.tabs[e].content.length; n++)
            if (null != a.tabs[e].content[n].name & null != a.tabs[e].content[n].value) switch (a.tabs[e].content[n].type) {
              ...
              case "inputNumber":
                ...
                break;
              case "singleSelect":
                a.tabs[e].content[n].value = parseFloat(t[a.tabs[e].content[n].name]);
                break;
              case "multitypes":
              case "categoriesSelect":
                ...
                break;
              case "select":
                t[a.tabs[e].content[n].name] ? a.tabs[e].content[n].value = t[a.tabs[e].content[n].name] : a.tabs[e].content[n].value = null;
                break;
              default:
                a.tabs[e].content[n].value = t[a.tabs[e].content[n].name]
            }
        var o = [];
        for (e = 0; e < a.tabs.length; e++)
          for (n = 0; n < a.tabs[e].content.length; n++)
            if (null != a.tabs[e].content[n].name & null != a.tabs[e].content[n].value) switch (a.tabs[e].content[n].type) {
              case "singleSelect":
                var s = a.tabs[e].content[n].value;
                o.indexOf(s) < 0 && o.push(s);
                break;
              case "multitypes":
                for (var i = 0; i < a.tabs[e].content[n].types.length; i++)
                  if ("singleSelect" == a.tabs[e].content[n].types[i].type)
                    for (var r = 0; r < a.tabs[e].content[n].value.length; r++) {
                      s = a.tabs[e].content[n].value[r][a.tabs[e].content[n].types[i].name];
                      o.indexOf(s) < 0 && o.push(s)
                    }
            }
    ...
    methods: {
      updateOption: function() {
        for (var t = {}, e = 0; e < this.tabs.length; e++)
          for (var n = 0; n < this.tabs[e].content.length; n++) null != this.tabs[e].content[n].name & null != this.tabs[e].content[n].value && (t[this.tabs[e].content[n].name] = this.tabs[e].content[n].value);
        var a = this;
        jQuery.ajax({
          url: pandastudio_framework.route + "pandastudio/framework/update_option",
          type: "POST",
          beforeSend: function(t) {
            t.setRequestHeader("X-WP-Nonce", pandastudio_framework.nonce)
          },
      ...
      mediaUpload: function(t, e, n) {
        ...
        o.open(), o.on("select", function() {
          ...
          for (var s = 0; s < a.tabs.length; s++)
            for (var i = 0; i < a.tabs[s].content.length; i++) a.tabs[s].content[i].name == t && ("" != n ? a.tabs[s].content[i].value[e][n] = value : a.tabs[s].content[i].value = value)
        })
      },
      multiMediaUpload: function(t) {
        ...
        n.open(), n.on("select", function() {
          ...
          for (s = 0; s < e.tabs.length; s++)
            for (var i = 0; i < e.tabs[s].content.length; i++) e.tabs[s].content[i].name == t && ("string" == typeof e.tabs[s].content[i].value ? e.tabs[s].content[i].value = o : e.tabs[s].content[i].value = e.tabs[s].content[i].value.concat(o))
        })
      },
      ...
      removeAllMultiUpload: function(t) {
        for (var e = 0; e < this.tabs.length; e++)
          for (var n = 0; n < this.tabs[e].content.length; n++) this.tabs[e].content[n].name == t && (this.tabs[e].content[n].value = "")
      },
      categorySelect: function() {
        return pandastudio_framework.categorySelector
      },
      querySinglePosts: function(t) {
        "" !== t ? (this.singleSelect.loading = !0, _this = this, setTimeout(function() {
          jQuery.ajax({
            url: pandastudio_framework.route + "pandastudio/framework/wp_query",
            type: "POST",
            beforeSend: function(t) {
              t.setRequestHeader("X-WP-Nonce", pandastudio_framework.nonce)
            },
            ...
            _this.singleSelect.query = t, _this.singleSelect.loading = !1
          }).fail(function() {
            _this.singleSelect.query = [], _this.singleSelect.loading = !1
          })
        }, 200)) : this.singleSelect.query = []
      },
      queryExistValue: function(t, e, n) {
        t && (this.singleSelect.query = [], this.singleSelect.loading = !0, _this = this, jQuery.ajax({
          url: pandastudio_framework.route + "pandastudio/framework/wp_query",
          type: "POST",
          beforeSend: function(t) {
            t.setRequestHeader("X-WP-Nonce", pandastudio_framework.nonce)
          ...
          n ? _this.singleSelect.query.push(t[0]) : _this.singleSelect.query = t
        }).fail(function() {
          n || (_this.singleSelect.query = [])
        }).always(function() {
          _this.singleSelect.loading = !1
        }))
      },
      showSaveBtn: function(t) {
        for (var e = !1, n = t.length - 1; n >= 0; n--) e |= null != t[n].value;
        return e
      },
      ...
      uploadData: function() {
        window.fileInput = document.createElement("input"), fileInput.type = "file", fileInput.accept = ".json", fileInput.onchange = this.readFile;
        fileInput.click()
      },
      readFile: function() {
        ...
        if (t) {
          ...
          e.onload = function() {
            try {
              ...
              if (t) {
                for (var e = {}, a = 0; a < n.tabs.length; a++)
                  for (var o = 0; o < n.tabs[a].content.length; o++) null != n.tabs[a].content[o].name & null != n.tabs[a].content[o].value && (e[n.tabs[a].content[o].name] = "");
                var s = 0,
                  i = 0;
                for (var r in e)
                  for (var l in i++, t)
      ...
      downloadData: function() {
        ...
        jQuery.ajax({
          ...
          var e = new Date,
            n = e.getFullYear().toString() + (e.getMonth() + 1).toString() + e.getDate().toString() + e.getHours().toString() + e.getMinutes().toString() + e.getSeconds().toString(),
      ...
      clearData: function() {
        ...
        this.$confirm("此操作将清空本主题的所有设置选项", "请确认", {
          ...
          for (var e = {}, n = 0; n < t.tabs.length; n++)
            for (var a = 0; a < t.tabs[n].content.length; a++) null != t.tabs[n].content[a].name & null != t.tabs[n].content[a].value && (e[t.tabs[n].content[a].name] = "");
          jQuery.ajax({
            url: pandastudio_framework.route + "pandastudio/framework/update_option",
            type: "POST",
            beforeSend: function(t) {
