...
add_action('rest_api_init', function () {
    register_rest_route('pandastudio/framework', '/get_post_meta/', array('methods' => 'post','callback' => 'get_post_meta_by_RestAPI',));
    register_rest_route('pandastudio/framework', '/update_post_meta/', array('methods' => 'post','callback' => 'update_post_meta_by_RestAPI',));
});
function get_post_meta_by_RestAPI($data)
{
    $ajaxData = json_decode($data->get_body(), true);
    $post_id = $ajaxData['postID'];
    $dataArray = $ajaxData['postMeta'];
    if (!$post_id) {
    ...
    foreach ($dataArray as $meta_name => $value) {
        $return[$meta_name] = get_post_meta($post_id, $meta_name, true) ? get_post_meta($post_id, $meta_name, true) : "";
    }return $return;
...
function pandastudio_framework_create_json_meta()
{
    ...
    foreach ($wp_all_post_types as $post_type) {
        if (in_array($post_type, $meta_screens)) {
...
}function pandastudio_framework_create_json_meta_box()
{
    ...
    echo '<script type="text/javascript" src="'.get_stylesheet_directory_uri().'/pandastudio_framework/assets/js/vue.js"></script><script type="text/javascript" src="'.get_stylesheet_directory_uri().'/pandastudio_framework/assets/js/element-ui.js"></script><link rel="stylesheet" type="text/css" href="'.get_stylesheet_directory_uri().'/pandastudio_framework/assets/css/'.$color.'.css"><link rel="stylesheet" type="text/css" href="'.get_stylesheet_directory_uri().'/pandastudio_framework/assets/css/rewrite_post_screen.css"><link rel="stylesheet" type="text/css" href="'.get_stylesheet_directory_uri().'/pandastudio_framework/assets/css/font-awesome.css"><div id="vue_rest">';?><template><span v-if="false" style="color: red;">您的浏览器不支持ECMAScript 5，请更换至IE9及以上版本</span></template><template><el-tabs v-model="tabIndex" v-loading="loading" v-show="show" class="panda_framework_metabox_tab" type="card"><el-tab-pane v-for="tab in tabs" v-if="gear_show(tab.gear_name,tab.gear_value)"><span slot="label"><i :class="tab.icon" class="fa"></i> {{tab.title}}</span><el-form ref="form" :label-width="tab.labelWidth" style="padding-right: 15px;" onsubmit="return false;"><el-form-item v-for="component in tab.content" v-if="gear_show(component.gear_name,component.gear_value)"><span slot="label" v-html="component.label"></span>
                    ...
                    <div v-if=" component.type == 'select' "><el-select size="small" v-model="component.value" :placeholder="component.placeholder ? component.placeholder : '请选择'" clearable><el-option v-for="select in component.selects" :key="select.value" :label="select.label" :value="select.value"><span style="float: left">{{ select.label }}</span><span style="float: right; color: #C0CCDA; font-size: 13px">{{ select.description }}</span></el-option></el-select></div>
                    <div v-if=" component.type == 'multitypes' ">
                        <div v-if="component.displayAsTable == false"><el-form-item v-for="(subValue , index) in component.value" :label="component.subLabel + ' ' + (index+1)" :label-width="component.subLabelWidth">
                                <div v-for="subcomponent in component.types" style="margin-bottom: 5px;">
                                    ...
                                    <div v-if=" subcomponent.type == 'select' " class="subcomponent"><el-select size="small" v-model="subValue[subcomponent.name]" :placeholder="subcomponent.placeholder ? subcomponent.placeholder : '请选择'" clearable><el-option v-for="select in subcomponent.selects" :key="select.value" :label="select.label" :value="select.value"><span style="float: left">{{ select.label }}</span><span style="float: right; color: #C0CCDA; font-size: 13px">{{ select.description }}</span></el-option></el-select></div>
                        ...
                        <div v-if="component.displayAsTable == true"><el-table size="mini" :data="component.value" style="width: 100%" border><el-table-column type="index" width="55" fixed></el-table-column><el-table-column :prop="subcomponent.name" :label="subcomponent.label ? subcomponent.label : subcomponent.name" v-for="subcomponent in component.types" min-width="265"><template scope="scope">
                                        ...
                                        <div v-if=" subcomponent.type == 'select' " class="subcomponent"><el-select v-model="scope.row[subcomponent.name]" :placeholder="subcomponent.placeholder ? subcomponent.placeholder : '请选择'" size="small" clearable><el-option v-for="select in subcomponent.selects" :key="select.value" :label="select.label" :value="select.value"><span style="float: left">{{ select.label }}</span><span style="float: right; color: #C0CCDA; font-size: 13px">{{ select.description }}</span></el-option></el-select></div>
                ...
                </el-form-item></el-form></el-tab-pane></el-tabs></template></div>
<script type="text/javascript" src="<?php echo get_stylesheet_directory_uri() ?>/pandastudio_framework/assets/template/meta_rest.js?version=<?php echo wp_get_theme()->get('Version') ?>"></script><?php
}?><?php
?>